...
class Downloader:
    ...
    def __init__(
        self,
        settings: Optional[Union[DownloaderOptionalOptions,
                                 DownloaderOptions]] = None,
        ...
        self.scan_formats = self.settings["detect_formats"] or [
        ...
        if self.settings["scan_for_songs"]:
            ...
            for scan_format in self.scan_formats:
                ...
                found_files = gather_known_songs(
                    self.settings["output"], scan_format)

                logger.debug("Found %s %s files", len(
                    found_files), scan_format)
        ...
        for audio_provider in self.settings["audio_providers"]:
            ...
            if audio_class is None:
                raise DownloaderError(
                    f"Invalid audio provider: {audio_provider}")
    ...
    def download_multiple_songs(
        ...
        if self.settings["fetch_albums"]:
            ...
            logger.info(
                "Fetching %d album%s", len(
                    albums), "s" if len(albums) > 1 else ""
            )
    ...
    def search_and_download(self, song: Song) -> Tuple[Song, Optional[Path]]:
        ...
        if not (song.name and (song.artists or song.artist)) and not (
            ...
            logger.error("Song is missing required fields: %s",
                         song.display_name)
            self.errors.append(f"Song is missing required fields: {
                               song.display_name}")
        ...
        try:
            ...
            if file_exists and self.settings["overwrite"] == "metadata":
                ...
                if dup_song_paths:
                    ...
                    for old_song_path in dup_song_paths:
                        ...
                        try:
                            logger.info(
                                "Removing duplicate file: %s", old_song_path)
            ...
            if download_info is None:
                ...
                raise DownloaderError(
                    f"yt-dlp failed to get metadata for: {
            ...
            if temp_file.exists():
                try:
                    ...
                    raise DownloaderError(
                        f"Could not remove temp file: {
                            temp_file}, possible duplicate song"
                    ) from exc
